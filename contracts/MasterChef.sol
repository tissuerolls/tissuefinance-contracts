...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accRollPerShare;   // Accumulated ROLLs per share, times 1e12. See below.
    ...
    function pendingRoll(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accRollPerShare = accRollPerShare.add(rollReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accRollPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 rollReward = multiplier.mul(rollPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        roll.mint(devaddr, rollReward.div(10));
        roll.mint(address(this), rollReward);
        pool.accRollPerShare = pool.accRollPerShare.add(rollReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accRollPerShare).div(1e12);
